            ...
            <div ref={sliderRef} className="flex flex-row overflow-x-auto pb-4 -mx-4 px-4 scroll-smooth no-scrollbar">
              <div className="flex flex-nowrap gap-4">
                {isCategoriesLoading ? (
                  ...
                  <div className="text-red-500 text-center w-full">Error loading categories</div>
                ) : (
                  categories?.map((category, index) => (
                    <div key={index} className="flex-shrink-0 sm:w-fit ">
                      <div className="flex flex-col rounded-full bg-gray-100 px-8 py-5 shadow-md hover:shadow-lg transition-all duration-300 border border-gray-100 hover:border-gray-100 transform hover:-translate-y-1">
                        <h2 className="font-bold font-dm text-lg sm:text-xl text-gray-800 mb-1">{category.name}</h2>
                        <p className="text-xs sm:text-sm text-gray-600">({category.coursesCount}) courses available</p>
                      </div>
                    </div>
                  ))
                )}
